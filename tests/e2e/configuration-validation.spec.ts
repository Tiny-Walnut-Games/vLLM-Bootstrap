...
test.describe('Configuration Validation', () => {
  ...
  test('should validate models.conf structure', async () => {
    ...
    for (const section of requiredSections) {
      ...
      expect(
        ...
        `Invalid model format in ${section}: ${defaultModel}`,
      ).toBe(true);
    }
  });
  ...
  test('should validate ports.conf structure', async () => {
    ...
    for (const [tier, expectedRange] of Object.entries(expectedRanges)) {
      expect(
        content.includes(`${tier} = ${expectedRange}`),
        `Missing or incorrect range for ${tier}`,
      ).toBe(true);
    }
  ...
  test('should validate chat-templates.conf structure', async () => {
    ...
    for (const modelName of modelNames) {
      if (modelName && modelName !== '') {
        expect(
          ...
          `Missing template mapping for model: ${modelName}`,
        ).toBe(true);
      }
    }
    ...
    for (const line of templateLines) {
      ...
      if (trimmed.includes('=') && !trimmed.startsWith('#')) {
        ...
        if (model && template) {
          expect(
            validTemplateTypes.includes(template),
            `Unknown template type: ${template} for model: ${model}`,
          ).toBe(true);
        }
      }
    }
  ...
  test('should validate HuggingFace authentication', async () => {
    try {
      const whoami = execSync('huggingface-cli whoami', {
        encoding: 'utf8',
        stdio: 'pipe',
      });
      // eslint-disable-next-line no-console
  ...
  test('should validate configuration consistency', async () => {
    ...
    for (const tier of portTiers) {
      expect(
        modelTiers.includes(tier),
        `Port tier ${tier} has no corresponding model section`,
      ).toBe(true);
    }
  });
