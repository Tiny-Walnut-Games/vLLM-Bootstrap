...
test.describe('Rider AI Assistant Integration', () => {
  ...
  test('Test Code Generation Capabilities', async ({ request }) => {
    ...
    const codePrompts = [
      {
        language: 'C#',
        prompt: 'Create a simple REST API controller for managing users with GET and POST endpoints.',
    ...
    const setupRecommendations = [
      {
        ...
        models: [
          { tier: 'fast', port: 8100, purpose: 'Autocomplete, quick suggestions' },
          { tier: 'edit', port: 8300, purpose: 'Code refactoring, documentation' },
      ...
      },
      {
        ...
        models: [
          { tier: 'plan', port: 8700, purpose: 'Architecture decisions, complex planning' },
